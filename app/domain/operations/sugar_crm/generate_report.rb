...
module Operations
  module SugarCRM
    # This operation generates a report of transmittable data for a given date and limit.
    class GenerateReport
      ...
      def construct_transmittable_data(params)
        transmittable_data = []
        families_range = Family.where(created_at: params[:date].all_day)
        families = families_range.order(created_at: :desc).limit(params[:limit]).to_a
        families.each do |family|
          fetch_transmittable_data(family, transmittable_data)
        end
      ...
      def fetch_transmittable_data(family, transmittable_data)
