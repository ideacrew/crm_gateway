...
RSpec.describe ::Operations::Transformers::FamilyTo::AccountAndContacts, type: :model, dbclean: :after_each do
  ...
  context 'transforms family payload into account/contacts ready data' do
    it 'should return success with proper accounts/contact information' do
      ...
      contact_contract = Crms::Contacts::ContactContract.new.call(result[:contacts].first)
      expect(contact_contract.errors.to_h).to eq({})
      account_contract = Crms::Accounts::AccountContract.new.call(result.except(:contacts))
      expect(account_contract.errors.to_h).to eq({})
    end
  end
end
